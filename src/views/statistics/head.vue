<template>
  <section class="headStatics-page">
    <div class="filter-wrapper">
      <!--    业务过滤-->
      <div class="flex-common">
        <van-field
          v-model="userListForm.select"
          size="large"
          placeholder="选择业务" readonly="readonly"
          @click="selectPop = true"
        />
        <van-icon name="arrow-down" class="arrow-down"/>
        <van-popup v-model="selectPop" position="bottom" :overlay="true">
          <ul class="filter-list">
            <li v-for="(item, index) in selectList"
                :key="index"
                @click="selectNavigate(item.value)"
            >
              {{ item.name }}
            </li>
          </ul>
    ...
    <div class="list-wrapper">
      <ul v-if="userListForm.select == '零售'" class="cityList">
        <li v-for="(item, index) in cityList"
            :key="index" @click="handleNavigate(item.code)"
        >
          <div class="item-left">
            {{ item.name }}
          </div>
      ...
      <p v-if="!cityList.length" class="no-data">暂无数据...</p>
    </div>
  </section>
</template>
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        userListForm: {
            // 设置默认时间
          // end_time: new Date().getFullYear()+"年"+(new Date().getMonth() + 1)+"月",
          select: "零售",
          end_time: ""
        },
        endTimePop: false,
    ...
    methods: {
      endTimeChange(e) {
        ...
        if (e) {
          if (this.userListForm.select == "零售") {
            // 格式化时间
            ...
            this.$api.material.getOrderCount(starTiime, end, 5).then(res => {
            ...
            // 列表的显示
            this.$api.material.getOrderCount(starTiime, end, 6).then(res => {
              ...
              for (let i = 0; i < payload.length; i++) {
                if (payload[i].name) {
                  ...
                  obj1.name = payload[i].name;
                  obj1.value = payload[i].orderCount;
                  obj1.code = payload[i].code;
                  this.cityList.push(obj1)
                }
              }
            });
          }
          if (this.userListForm.select == "大客户") {
            // 格式化时间
            let years = e.getFullYear();
            let months = e.getMonth() + 1;
            let endTimeArr = [];
            ...
            let dateYar = dateString.toLocaleDateString();
            let end = dateYar.replace(new RegExp('/', 'g'), "-") + " 23:59:59.999";
            this.$api.material.getDiDiOrderCount(starTiime, end, 1).then(res => {
      ...
      handleNavigate (page) {
        this.$router.push({ name: "platformStatics",
          query: {
            id: page,
  ...
  .headStatics-page {
    ...
    .filter-list {
      ...
      & > li {
        ...
        border-bottom: 0.027rem  /* 1/37.5 */ solid #F7F7F7;
        &:last-child {
          border-bottom: none;
        }
      }
